...
export default [
     ...
     */
    {
      ...
      content: [
         ...
         * 同时提供以下几种样式配置方法
         */
        p(`**阿里巴巴（杭州）· 淘宝前端团队（FED）**`, `*2019.5 至今*`),
        p(`*实习生* ...`),
        p(`**腾讯（深圳）PCG · 社交娱乐产品部**`, `*2019.1 - 2019.3*`),
        p(`**实习生** 十余人的纯前端组，技术栈涉及 Node 较多。最大收获是体验到了公司内的开发流程及规范，以及多人协作、需求评审等校内很难锻炼到的能力。共参与了 3 个项目的开发：
          - *内部运营系统：* Node 直出项目，MVC，负责开发了几个页面。
          - *手Q 某 DAU 2000w H5 项目：* 项目采用内部自研 NodeJS Server 并配有专门的配置平台。我负责开发了一个活动下线页中间件，可根据配置项识别 Request URL 并跳转到指定页面。中途为了性能优化写了个配置项缓存类，后面经过近一步完善（支持多进程复用）后发布到了团队公共组件库中。
          - *企业微信机器人消息模块：* **个人独立负责。** 帮助管理人员在操作配置平台时周知其他成员，以及快捷查找组内文档。自己实现的订阅者模式，支持简易的自定义模版，支持简易的模糊搜索。`),
        p(`重庆邮电大学 · 红岩网校工作站`, `2017.6 至今`),
        p(`*Web 研发部“技术总监”<small>（2018.6 至今）</small>* 工作站成立于2000年，共有成员80余人（Web 研发部 20+），拥有 PRD-UI-前后端开发-运营-迭代的完整开发流程。曾主持开发了网校内部前端项目通用的工具库以及 3 个完整项目，同时参与上线维护、重构代码、项目迁移/部署等任务。`),
        p(`[Source](https://github.com/CoderMing/Welcome2017) [Link](http://hongyan.cqupt.edu.cn/welcome/2017) *重邮新生专题网 · PC 端网页：* 此项目是一年一版、拥有完整开发流程的桌面端页面，我为 2017 版负责人。开发中使用了很多 CSS3 的特性，同时在开发过程中与产品视觉进行沟通，对界面进行了精细的优化。 **最终上线一个月 UV 6k+，PV 2w+。**`),
        p(`*重庆邮电大学 · 校招生办公室*`, `*2017.5 至今*`),
        p(`*开发者* 与老师一起做小程序以及每年的新生数据分析等事物。这也是我在大学开始独立承担任务的地方。
          - **重庆邮电大学招生办公室 · 微信小程序：** 为提高高考填报志愿期间学校的服务质量，与老师合作开发的小程序。采用 Flex 弹性布局， **2017 年 6 月上线至今共有 6w+ 用户、录取工作期间 DAU 3.3k+、总 PV 520w+，并成为 “腾讯微校－高校微信西部明星” 之一。** 这个项目技术实现并不难，但它让当时（大一）的我学会如何制作一个完整的项目，让我踏入了实践的大门。目前主要的任务为后期维护。`),
        p(`*重庆邮电大学 · 计算机通信实验室*`, `*2016.12 - 2017.6*`),
        p(`*实验室成员* 参与开发前端 4 人后端 10 人的项目。使用 Git 进行版本控制。当时（大一）资历尚浅只是打杂，主要收获是对前端有了更全面的认识、确定了自己的学习路径。`)
      ]
    },
    /**
     * 每一个小模块，都有以下几个配置项：
