...
export default [
     ...
     */
    {
      ...
      content: [
        ...
        p(`- *QAP 转译小程序工具<small>（开发中）</small>：* 此工具目的是为尽快统一千牛端应用（QAP）至小程序体系（类似于 Taro 跨端方案）。编写 babel 插件静态转换 AST、胶水层来适配组件和 SDK，现已实现部分 QAP 应用的转译。`),
        p(`- *商家应用开发者平台：* React Web 端应用，使用 React Hooks 进行开发。一期项目拥有 80+ 页面，图表非常多。前半个月角色为参与者，之后 owner 了整个项目的前端一个月。使用 AntV 的过程中遇到了一个框架层面的 bug，贡献了 bugfix。`),
        h(`**腾讯（深圳）PCG · 社交娱乐产品部**`, `*2019.1 - 2019.3*`),
        p(`**实习生** *（英文名 demingzhang）* 十余人的纯前端组，业务为 QQ钱包 & 厘米秀。技术栈涉及 Node 较多。 **在 KM 上留下了两篇技术文章** 。共参与了 3 个项目的开发：`),
        ...
        p(`- **重庆邮电大学招生办公室 · 微信小程序：** 为提高学校高考招生的服务质量，与老师合作开发的小程序。采用 Flex 弹性布局。 **2017 年 6 月上线至今共有 7w+ 用户、录取工作期间 DAU 3.3k+、总 PV 520w+，并成为 “腾讯微校－高校微信西部明星” 之一。** 这个项目并不复杂，是我的第一个（大一暑假）项目。`),
    ...
    },
    {
      ...
      content: [
        ...
        p('- *简单了解并实践过* **前端工程化。** 写过 Webpack、Gulp、 Babel 的插件（自用）、写过简单的前端自动化脚本、发布过 npm 包。使用过CSS 预处理器、Lint、HMR 并简单了解其原理。'),
        ...
        p('- **较为熟悉小程序。** 大一（2017）开始接触及开发小程序，目前的实习也主要参与小程序相关的开发，简单了解小程序多线程间通信、jsbrige、数据更新机制等相关知识。'),
